"""
Retry calls for first 100 rows where transfer column is empty
"""
import sys
import os
...
# Configuration
MAX_ROWS = 100  # First 100 rows
CALL_INTERVAL_SECONDS = 36  # 36 seconds between calls

if __name__ == "__main__":
    print("=" * 80)
    print("📞 RETRY CALLS FOR ROWS WITH EMPTY TRANSFER STATUS")
    print("=" * 80)
    print(f"📊 Checking first {MAX_ROWS} rows")
    ...
    if not auto_confirm:
        ...
        try:
            response = input(f"Proceed with retry calls for rows with empty transfer status? (y/N): ").strip().lower()
    ...
    try:
        ...
        # Get all customers from first MAX_ROWS rows
        print("\n" + "=" * 80)
        print("🔍 FETCHING CUSTOMERS (FIRST 100 ROWS)")
        print("=" * 80)
        all_customers = smartsheet_service.get_all_customers_with_stages()
        
        # Filter to first MAX_ROWS rows
        first_rows = all_customers[:MAX_ROWS]
        print(f"📊 Total customers in sheet: {len(all_customers)}")
        print(f"📊 Checking first {len(first_rows)} rows")
        
        # Filter customers where transfer column is empty
        # Check multiple possible column names
        customers_to_retry = []
        for customer in first_rows:
            transfer_status = (
        ...
        print(f"⏰ Target completion: {(datetime.now(pacific_tz) + timedelta(seconds=len(customers_to_retry) * CALL_INTERVAL_SECONDS)).strftime('%I:%M %p %Z')}")
        print("=" * 80)
        ...
        for i, customer in enumerate(customers_to_retry, 1):
            ...
            # Stop if too close to end time (within last 5 minutes)
            ...
            try:
                ...
                import traceback
                traceback.print_exc()
                # Try to get more error details from VAPI service
                if hasattr(e, 'response') or '400' in str(e):
                    print(f"   💡 This might be a validation error (400)")
                    print(f"   💡 Check phone number format or customer data")
                total_failed += 1
