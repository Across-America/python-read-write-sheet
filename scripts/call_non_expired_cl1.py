...
注意:
    - 如果没有指定日期，脚本会提示用户输入
    - 只拨打未过期的保单（Cancellation Date > F/U Date）
...
def call_non_expired_customers(target_date_str=None, auto_confirm=False):
    """
    拨打未过期的保单客户
    ...
    if total_customers == 0:
        print(f"\n✅ 没有 F/U Date = {target_date} 的未过期客户需要拨打")
        return True
    
    # 显示摘要并询问确认
    print(f"\n{'=' * 80}")
    print(f"📊 准备拨打的未过期客户列表 (F/U Date = {target_date}):")
    print(f"{'=' * 80}")
...
if __name__ == "__main__":
    ...
    call_non_expired_customers(target_date_str=target_date_str, auto_confirm=auto_confirm)
